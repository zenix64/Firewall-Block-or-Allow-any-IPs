...
param ($InputFile = "blocklist.txt", $action="block", $RuleName="Test", $ProfileType = "any", $protocol= "any", $localport= "any", $InterfaceType = "any", [Switch] $DeleteOnly)
...
else #to be  changed
{
    ...
    if ($protocol -eq "any") 
    {
        Write-Host "You should specify the protocol ex. UDP or TCP"
    ...
    else 
    {
        ...
        do {
            ...
            "`nCreating an inbound firewall rule named '$rulename-#$icount' for IP ranges $start - $end" 
            netsh.exe advfirewall firewall add rule name="$rulename-#$icount" dir=in action="$action" localip=any remoteip="$textranges" description="$description" profile="$profiletype" interfacetype="$interfacetype" protocol="$protocol" localport="$localport"
            if (-not $?) { "`nFailed to create '$rulename-#$icount' inbound rule for some reason, continuing anyway..."}
    
            "`nCreating an outbound firewall rule named '$rulename-#$icount' for IP ranges $start - $end" 
            netsh.exe advfirewall firewall add rule name="$rulename-#$icount" dir=out action="$action" localip=any remoteip="$textranges" description="$description" profile="$profiletype" interfacetype="$interfacetype" protocol="$protocol" localport="$localport"
            if (-not $?) { "`nFailed to create '$rulename-#$icount' outbound rule for some reason, continuing anyway..."}
